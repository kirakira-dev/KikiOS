/*
 * Kotos - KikiOS Image Viewer
 *
 * View BMP, PNG, JPG images. Based on the KikiOS viewer.
 * This is essentially a renamed viewer with the Kotos branding.
 */

#include "../lib/kiki.h"
#include "../lib/gfx.h"

// abs() needed by stb_image
...
static void scan_directory(void) {
    ...
    while (api->readdir(dir, index, name, sizeof(name), &type) == 0) {
        if (type == 1 && is_image_file(name) && file_count < MAX_FILES) {
            ...
            if (strcmp(name, current_filename) == 0) current_file_index = file_count;
            file_count++;
        }
        index++;
    }
...
static int load_image(const char *path) {
    ...
    if (!file || api->is_dir(file)) return -1;
    int size = api->file_size(file);
    if (size <= 0) return -1;
    uint8_t *file_data = api->malloc(size);
    if (!file_data) return -1;
    if (api->read(file, (char *)file_data, size, 0) != size) { api->free(file_data); return -1; }
    int channels;
...
static void draw_image(void) {
    buf_fill_rect(0, 0, win_w, win_h, 0x282828);
    if (!image_data) {
        buf_draw_string(10, win_h / 2, "No image loaded", COLOR_WHITE, 0x282828);
        api->window_invalidate(window_id);
        return;
    }
    int avail_w = win_w - 4, avail_h = win_h - 4;
    int draw_w = img_width, draw_h = img_height;
    ...
    for (int y = 0; y < draw_h; y++) {
        ...
        for (int x = 0; x < draw_w; x++) {
            ...
            uint32_t color = (image_data[src_idx] << 16) | (image_data[src_idx + 1] << 8) | image_data[src_idx + 2];
            int dst_x = start_x + x;
...
int main(kapi_t *kapi, int argc, char **argv) {
    ...
    if (argc < 2) {
        out("Kotos - KikiOS Image Viewer\n");
        out("Usage: kotos <image>\n");
        out("Supports: PNG, JPG, BMP\n");
        return 1;
    }
    if (!api->window_create) {
        out("kotos: requires GUI (run from desktop)\n");
        return 1;
    }
    if (load_image(argv[1]) != 0) {
        out("kotos: failed to load: ");
        out(argv[1]);
        out("\n");
        return 1;
    }
    int content_w = img_width, content_h = img_height;
    ...
    window_id = api->window_create(win_x, win_y, content_w, content_h + 18, "Kotos");
    if (window_id < 0) {
        out("kotos: failed to create window\n");
    ...
    while (running) {
        ...
        while (api->window_poll_event(window_id, &event_type, &data1, &data2, &data3)) {
            switch (event_type) {
                case WIN_EVENT_CLOSE: running = 0; break;
                case WIN_EVENT_KEY:
                    if (data1 == 'q' || data1 == 'Q' || data1 == 27) running = 0;
                    else if (data1 == KEY_LEFT) navigate(-1);
                    else if (data1 == KEY_RIGHT) navigate(1);
                    break;
